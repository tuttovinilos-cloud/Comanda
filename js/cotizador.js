console.log("COTIZADOR JS conectado v35 pdf limpio corporativo");
...
async function crearDocumentoPDF(snapshot = crearSnapshotActual()){
  ...
  const azul = [21,59,255];
  const azulOscuro = [11,31,122];
  const azulSuave = [239,243,255];
  const linea = [217,222,234];
  const texto = [17,24,39];
  const gris = [92,100,112];
  ...
  if(logoDataUrl){
    try{
      doc.addImage(logoDataUrl, 'PNG', 9, 7, 46, 13.2, undefined, 'FAST');
    }catch(error){
      console.warn('No se pudo insertar el logo en el PDF:', error);
      doc.setFont('helvetica','bold');
      doc.setFontSize(12);
      doc.setTextColor(255,255,255);
      doc.text('TUTTO VINILOS', 12, 14);
    }
  }else{
    doc.setFont('helvetica','bold');
    doc.setFontSize(12);
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(255,255,255);
  doc.text('Tel: 0414-414-3004', W-14, 7.5, {align:'right'});
  doc.text('Email: tuttovinilos@gmail.com', W-14, 12.2, {align:'right'});
  doc.text('RIF: J-40218250-3', W-14, 16.9, {align:'right'});

  // TÍTULO CENTRAL
  doc.setFillColor(...azulSuave);
  doc.rect(0, 27, W, 13, 'F');
  doc.setFont('helvetica','bold');
  doc.setTextColor(...azulOscuro);
  doc.setFontSize(12.5);
  doc.text((form.tipo || 'Cotización').toUpperCase(), W/2 - 8, 35.2, {align:'right'});
  doc.setFont('helvetica','normal');
  doc.setTextColor(...texto);
  doc.setFontSize(13);
  doc.text(form.numero || '', W/2 - 5, 35.2, {align:'left'});
  ...
  clientRows.forEach(([label, value]) => {
    ...
    y += Math.max(lineGap, lines.length * 4.6);
  });
  ...
  const tableStart = Math.max(y + 8, 88);
  doc.autoTable({
    startY: tableStart,
    ...
    columnStyles: {
      ...
      4:{halign:'center', cellWidth:31, fontStyle:'bold'}
    },
  ...
  const totalH = Number(t.iva || 0) > 0 ? 32 : 24;
  ...
  if(form.notas){
    const notesW = W - 28 - rightW - 10;
    doc.setDrawColor(...linea);
    doc.setFillColor(252,252,254);
    doc.roundedRect(14, fy, notesW, 30, 2.5, 2.5, 'FD');
    doc.setFont('helvetica','bold');
    doc.setFontSize(7.5);
    doc.setTextColor(...azulOscuro);
    doc.text('NOTAS / CONDICIONES', 17, fy + 5.2);
    doc.setFont('helvetica','normal');
    doc.setFontSize(8);
    doc.setTextColor(...gris);
    const lines = doc.splitTextToSize(form.notas, notesW - 7);
    doc.text(lines.slice(0,5), 17, fy + 10.5);
  }

  doc.setDrawColor(...linea);
  doc.setFillColor(255,255,255);
  doc.rect(rightX, fy, rightW, 8, 'FD');
  doc.setFont('helvetica','normal');
  doc.setFontSize(8.4);
  doc.setTextColor(...texto);
  doc.text('Sub Total:', rightX + 4, fy + 5.4);
  doc.setFont('helvetica','bold');
  doc.text(currency(t.subtotal), rightX + rightW - 4, fy + 5.4, {align:'right'});

  let ty = fy + 8;
  if(Number(t.iva || 0) > 0){
    doc.setFillColor(255,255,255);
    doc.rect(rightX, ty, rightW, 8, 'FD');
    doc.setFont('helvetica','normal');
    doc.setTextColor(...texto);
    doc.text('IVA 16%:', rightX + 4, ty + 5.4);
    doc.setFont('helvetica','bold');
    doc.text(currency(t.iva), rightX + rightW - 4, ty + 5.4, {align:'right'});
    ty += 8;
  }

  doc.setFillColor(...azul);
  doc.rect(rightX, ty, rightW, 12, 'F');
  doc.setFont('helvetica','bold');
  doc.setFontSize(12);
  doc.setTextColor(255,255,255);
  doc.text('TOTAL', rightX + 4, ty + 8);
  doc.text(currency(t.total), rightX + rightW - 4, ty + 8, {align:'right'});
