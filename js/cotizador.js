console.log("COTIZADOR JS conectado v29");
...
async function cargarCotizacionesPrevias(){
  ...
  const { data:rows, error } = await db()
    .from("cotizaciones")
    .select("id,fecha,numero,tipo_documento,cliente,telefono,total,pdf_url,created_at")
...
function renderCotizacionesPrevias(){
  ...
  tbody.innerHTML = lista.map(c => {
    const pdf = c.pdf_url
      ? `<a class="prev-link" href="${html(c.pdf_url)}" target="_blank" rel="noopener">Abrir PDF</a>`
    ...
    return `
      <tr>
        ...
        <td><div class="prev-client">${html(c.cliente || "—")}</div></td>
