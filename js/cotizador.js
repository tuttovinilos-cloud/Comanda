console.log("COTIZADOR JS conectado v63 cliente autofill robusto + propuesta compacta");
...
async function cargarClientesCotizador(){
  ...
  /*
    FIX v63:
    Para el autocompletado del cotizador no usamos un select limitado.
    La falla venía de pedir columnas que no siempre existen y luego caer a
    un select básico que no traía rif_cedula ni direccion. Con select("*")
    Supabase trae todas las columnas reales de la tabla clientes y el campo
    Cliente puede rellenar RIF, teléfono, correo y dirección de inmediato.
  */
  let res = await db()
    .from("clientes")
    .select("*")
  ...
  if(res.error){
    console.warn("Clientes select * falló. Probando columnas mínimas:", res.error);

    res = await db()
      .from("clientes")
      .select("id,nombre,tipo_cliente,telefono,correo,rif_cedula,direccion,notas,activo")
      .order("nombre", { ascending:true });
  }
...
function guardarClienteEnCache(cliente){
  if(!cliente || !cliente.nombre) return cliente;

  const id = Number(cliente.id);
  const idx = clientesDB.findIndex(c => {
    if(Number.isFinite(id) && Number(c.id) === id) return true;
    return normalizar(c.nombre) === normalizar(cliente.nombre);
  });

  if(idx >= 0){
    clientesDB[idx] = { ...clientesDB[idx], ...cliente };
  }else{
...
async function buscarClienteEnSupabasePorNombre(nombre){
  const limpio = String(nombre || "").trim();
  if(!limpio || !validarSupabase()) return null;
  ...
  if(error){
    console.warn("No se pudo buscar cliente exacto en Supabase:", error);
    return null;
  }

  const n = normalizar(limpio);
  const exacto = (rows || []).find(c => normalizar(c.nombre) === n) || null;

  if(exacto){
    guardarClienteEnCache(exacto);
  }
...
function llenarDatosCliente(cliente){
  ...
  if(rif) rif.value = pickClienteField(cliente, "rif");
  if(telefono) telefono.value = pickClienteField(cliente, "telefono");
  if(email) email.value = pickClienteField(cliente, "correo");
  if(direccion) direccion.value = pickClienteField(cliente, "direccion");
...
function bindEvents(){
  ...
  if(clienteInput){
    let clienteAutofillToken = 0;

    const autocompletarClienteActual = async () => {
      const token = ++clienteAutofillToken;
      const valor = clienteInput.value;
      const mini = $("clienteMini");
      let cliente = buscarClientePorNombre(valor);
      ...
      if(mini){
        mini.textContent = valor.trim()
          ? "Cliente nuevo: se guardará automáticamente en clientes."
          : "Escribe para buscar o crear cliente nuevo.";
      }

      // Refuerzo: si el cliente existe pero no quedó en el cache local,
      // se consulta directo a Supabase y se llenan los datos sin esperar blur.
      cliente = await buscarClienteEnSupabasePorNombre(valor);

      if(token !== clienteAutofillToken) return;
